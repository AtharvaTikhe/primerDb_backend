...
def run_tabix(db_dict, id):
    ...
    for db_name, db_details in db_dict[id].items():
        ...
        if proc_f.returncode == 0 and proc_r.returncode == 0:
            ...
            print(f'forward primer process returned : {db_details["cmd"][0]} {proc_f.returncode}')
            print(f'reverse primer process returned : {db_details["cmd"][0]} {proc_r.returncode}')
...
class DbLookup:
    ...
    def __init__(self, positions: dict, id):
        ...
        self.parser = ParseResults(results)
...
get_res('chr13', "20189511", '1000', 'CHR13x2', '900,200', '1')
